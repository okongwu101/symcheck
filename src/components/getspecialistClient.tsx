...
import { diagnosisIDAtom, genderAtom, yearOfBirthAtom } from "@/lib/atoms"
...
export default function GetSpecialistsClient({ token }: { token: string }) {
    ...
    const { data: specialists } = useQuery<SpecialistsInterface[]>({
        queryKey: [`${process.env.NEXT_PUBLIC_SPECIALIZATION_BASE}token=${token}&symptoms=[${diagnosisID}]&gender=${gender}&year_of_birth=${year}&format=json&language=en-gb`],
